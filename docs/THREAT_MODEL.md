...
|---|---|---|
| Malicious storage node / modified chunk | CID before decrypt, AEAD, plaintext hash, fallback | planned |
...
| Replay attacker | one-use challenges, request nonce/expiry, certificate binding | planned |
...
| Quota/slowloris/oversize | integer quotas, frame/blob bounds, timeouts/cancellation | planned |
| Malicious member/revoked credential | scoped roles, expiry, serial revocation at every entry | planned |
